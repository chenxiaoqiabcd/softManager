...
namespace DuiLib {
	...
	static std::vector<RGBA> convertPNGBufferToRGBA(const uint8_t* buffer, size_t bufferSize, int& width, int& height) {
		...
		if (SUCCEEDED(hr)) {
			hr = pFactory->CreateDecoderFromStream(
				SHCreateMemStream(buffer, static_cast<UINT>(bufferSize)),
				NULL,
		...
		if (SUCCEEDED(hr)) {
			UINT bufferSize = width * height * sizeof(RGBA);
			rgbaData.resize(bufferSize);

			hr = pConverter->CopyPixels(NULL, width * sizeof(RGBA), bufferSize, reinterpret_cast<BYTE*>(rgbaData.data()));
		}
	...
	static LPBYTE WINAPI svg_load_from_memory(const LPBYTE buffer, int &out_width, int &out_height, int hope_width, int hope_height, DWORD fillcolor, CPaintManagerUI* pManager)
	{
		...
		auto document = lunasvg::Document::loadFromData((const char*)buffer);

		auto ww = document->width();
		auto hh = document->height();

		out_width = pManager->GetDPIObj()->Scale(ww);
		out_height = pManager->GetDPIObj()->Scale(hh);

		auto lunbmp = document->renderToBitmap(out_width, out_height, fillcolor);
		lunbmp.convertToRGBA();

		int len;
		unsigned char* png = stbi_write_png_to_mem(
			(unsigned char*)lunbmp.data(),
			0,
			lunbmp.width(),
			lunbmp.height(), 4, &len);

		int ww1, hh2;
		auto vecOut = convertPNGBufferToRGBA(png, len, ww1, hh2);
		pImage = (LPBYTE)new char[vecOut.size()];
		memcpy(pImage, vecOut.data(), vecOut.size());
