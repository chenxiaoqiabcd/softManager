...
namespace DuiLib
{
	...
	int CTileLayoutUI::GetChildVPadding() const
	{
		...
		if (m_pManager) return m_pManager->GetDPIObj()->Scale(m_iChildVPadding);
		return m_iChildVPadding;
	}
	...
	void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			int iColumnIndex = index/m_nColumns;
			int iRowIndex = index%m_nColumns;
			int iPosX = rc.left + iRowIndex * (sz.cx + GetChildPadding());
			if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) {
				iPosX -= m_pHorizontalScrollBar->GetScrollPos();
			}
			int iPosY = rc.top + iColumnIndex*(sz.cy+GetChildVPadding());
			if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
