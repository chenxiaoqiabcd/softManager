...
#define II( a, b, c, d, x, s, ac ) { \
	...
	const byte* MD5::digest()
	{
		if (!_finished)
		{
			//		_finished = true;//注意该处的修改 final 调用了update函数 update函数赋值 _finished = false 所以此处的_finished 应放在final函数调用之后 以免执行该段代码
			final();
			_finished = true;
		}
	...
	std::string MD5::GetFileMd5(const wchar_t* szPath)
	{
		std::string strM;
		FILE* pf = _wfopen(szPath, L"rb");
		if (pf) {
			char buf[1024];
			size_t sr = fread(buf, 1, 1024, pf);

			MD5 md;
			while (sr != 0) {
				md.update(buf, sr);
				sr = fread(buf, 1, 1024, pf);
			}
			fclose(pf);
			strM = md.toStringA();
		}
