...
std::string LetterHelper::GetLetter(const char* value) {
	...
	for (int i = 0; i < len; i++)
	{
		...
		ucHigh = (unsigned char)value[i];
		ucLow = (unsigned char)value[i + 1];
...
std::wstring LetterHelper::GetLetter(const wchar_t* value) {
	return CStringHelper::a2w(GetLetter(CStringHelper::w2a(value).c_str()));
}

std::string LetterHelper::FindLetter(int nCode) {
	std::string strValue;
	switch (nCode)
	{
	case 6325:
